...
From us-west1-docker.pkg.dev/plainsightai-prod/oci/{{REPO_NAME_KEBABCASE}}-model:${RESOURCE_BUNDLE_VERSION} as model
# If Exported to Planisight Dev from protege
# From us-central1-docker.pkg.dev/plainsightai-dev/oci/{{REPO_NAME_KEBABCASE}}-model:${RESOURCE_BUNDLE_VERSION} as model
...
CMD ["python", "-m", "{{REPO_NAME_SNAKECASE}}.filter"]
