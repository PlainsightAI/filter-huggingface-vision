...
| `exemplar_embeddings_path` | string | — | No | For `embedding` type: path to `.npz` file with reference embeddings. Accepts a local path or a `gs://bucket/bank.npz` URI (read via workload identity / ADC — see [docs/embedding-gcs.md](docs/embedding-gcs.md)) |
