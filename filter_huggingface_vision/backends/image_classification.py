...
class ImageClassificationBackend(VisionBackend):
    ...
    def load(self, config):
        ...
        try:
            ...
            raise RuntimeError(
                f"Model '{model_id}' could not be loaded as an image-classification model "
                "(AutoImageProcessor + AutoModelForImageClassification). "
                f"Check that the model's architecture exposes an image-classification head: {repr(e)}"
            ) from e
        except Exception as e:
            raise RuntimeError(
                f"Unexpected failure loading '{model_id}@{revision}' for image classification: {repr(e)}"
            ) from e
